...
static rt_device_t baro_device_t;		//应用层 baro 设备
...
rt_err_t baro_collect_data(void *args)
{
	...
	const double T1 = 15.0 + 273.15;	/* temperature at base height in Kelvin, [K] = [癈] + 273.15 */
...
void ms5611_entry(void *parameter)
{
	...
	res |= rt_ms5611_init("spi_d3");					//初始化注册adxl355驱动程序
  
	baro_device_t = rt_device_find("ms5611"); //应用层查找adxl355设备
	
	if(baro_device_t == RT_NULL)								//失败处理
	{
	...
	while(1)
	{
			...
			if(sensor_baro_get_state() == S_COLLECT_REPORT)
			{
					...
					rt_kprintf("temp %d, press: %d\n",report_baro.altitude,report_baro.temperature);
			}
